...
//           axios.get("http://localhost:5000/api/reservation/myBookings", config), // <-- FIXED
...
const UserDashboard = () => {
  ...
  const fetchData = async () => {
    ...
    // Fetch verification status independently
    try {
      ...
      // Extract user info from verification record
      setUserInfo({
      ...
      // If verification fails, try to get user info from a separate endpoint
      // or extract from token, or set a default
      try {
        // Option 1: Call a separate user info endpoint
        const userResponse = await axios.get(
          "http://localhost:5000/api/users/profile", // or whatever your user endpoint is
          config
        );
        setUserInfo({
        ...
        // Option 2: You could decode the JWT token to get user info
